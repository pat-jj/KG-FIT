...
Update Mode 1 - Cluster A Covers Cluster B: cluster B belongs to cluster A. cluster B is a subcluster of cluster A. The name of cluster A should uniformly describe the entities from cluster A and the name of cluster B.
Update Mode 2 - Cluster B Covers Cluster A: cluster A belongs to cluster B. cluster A is a subcluster of cluster B. The name of cluster B should uniformly describe the entities from cluster B and the name of cluster A.
Update Mode 3 - Merge Cluster A and B: these two clusters can be merged. The name of two clusters should be similar and entities from two clusters should be similar.
Update Mode 4 - Create New Cluster C: these two clusters cannot be merged, and no cluster belongs to any other.

You need to select a update mode based on two clusters.
If you select mode 3, you should suggest a name of merged cluster. The new name should be similar to cluster A and B.
If you select mode 4, you should also suggest a name of new cluster. The new cluster name should uniformly describe two clusters.
...
def llm_name_cluster(entities):
    entities_str = ', '.join(entities)
    name = get_llm_response(NAME_CLUSTER_PROMPT.format(entities=entities_str))
    return name
...
def llm_split_cluster(cluster_name, entities):
    entities_str = ', '.join(entities)
    while True:
        try:
...
def llm_update_two_clusters(cluster_name_1, entities_1, cluster_name_2, entities_2):
...
def construct_bot_hierarchy(initial_hierarchy):
    current_cluster_id = 0

    def recursion_construct_bot_hierarchy(root):
        nonlocal current_cluster_id

        if isinstance(root, list):
            if len(root) < MIN_ENTITIES_IN_LEAF:
                return
            
            cluster_entities = root
            cluster_name = llm_name_cluster(cluster_entities)

            splitted_clusters = llm_split_cluster(cluster_name, cluster_entities)
...
def construct_top_hierarchy(initial_hierarchy):
    ...
    def recursion_construct_top_hierarchy(root):
        ...
        while True:
            update_mode, name = llm_update_two_clusters(right_node_value['name'], right_node_children, left_node_value['name'], left_node_children)
        ...
        if is_leaf:
            ...
            for key, child in root['children'].items():
                if child == []:
                    ...
                    new_root[key] = child
            if has_empty_list:
                new_root['Cluster_entity_list'] = empty_list
...
# main
def llm_refine_hierarchical_knowledge(initial_hierarchy):

    # 1. construct hierarchical knowledge below the clusters (bottom hierarchical knowledge)
    print('Constructing bottom hierarchy...')
    if os.path.exists('./step1_res.json'):
        with open('./step1_res.json') as f:
            tmp_hierarchy = json.load(f)
    else:
        tmp_hierarchy = construct_bot_hierarchy(initial_hierarchy)
        with open('./step1_res.json', 'w') as f:
    ...
    if os.path.exists('./step2_res.json'):
        with open('./step2_res.json') as f:
            refined_hierarchy = json.load(f)
    else:
        refined_hierarchy = construct_top_hierarchy(tmp_hierarchy)
        with open('./step2_res.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    with open('./refined_hier.json') as f:
    # with open('./test.json') as f:
        initial_hierarchy = json.load(f)
    refined_hierarchy = llm_refine_hierarchical_knowledge(initial_hierarchy)
    with open('./llm_refined_hier.json', 'w') as f:
        json.dump(refined_hierarchy, f)
